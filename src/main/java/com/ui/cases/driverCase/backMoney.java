package com.ui.cases.driverCase;

import com.ui.base.Base;
import com.ui.cases.driverCase.addDriverCase;
...
public class backMoney extends Base {
    ...
    @Test(description = "正常回收司机资金",priority = 0)
    public void backMoney() throws InterruptedException {
        ...
        Thread.sleep(2000);
        double yMoney = doubleNum(getText(getElement("回收司机资金", "余额")));
        Thread.sleep(1000);
        click(getElement("回收司机资金","更多"));
        ...
        Thread.sleep(1000);
        driver.navigate().refresh();
        double alyMoney = doubleNum(getText(getElement("回收司机资金", "余额")));
        ...
        //期望万金油余额
        double extotalMoney=totalMoney+doubleNum(money);
        double exqyqMoney=qyqmoney+doubleNum(money);
    ...
    @Test(description = "超额回收司机资金",priority = 1)
